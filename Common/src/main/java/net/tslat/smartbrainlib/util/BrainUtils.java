...
public final class BrainUtils {
    ...
    public static void addBehaviour(Brain<?> brain, int priority, Activity activity, Behavior behaviourControl) {
        ...
        brain.availableBehaviorsByPriority
				.computeIfAbsent(priority, priority2 -> Maps.newHashMap())
				.computeIfAbsent(activity, activity2 -> Sets.newLinkedHashSet()).add(behaviourControl);


        for (var memoryType : behaviourControl.entryCondition.keySet()) {
            brain.memories.putIfAbsent((MemoryModuleType<?>) memoryType, Optional.empty());
        }
    }
